...
**Where it stands (2026-09-26): designed, installed as a Claude Code subagent.** The full role prompt (verbatim from Ryan) lives in the repo at `.claude/agents/website-intelligence.md`; this note is the contract. It works **before** [[10_Agents/05_Website_App_General]]: it researches the business, audits the current site, benchmarks competitors, mines public reviews and hands over a `WEBSITE_BUILD_BRIEF.json`. Not yet an n8n workflow — it runs in a Claude Code session on Ryan's request (see §11 for the commands). Output folder template: `80_Clients/_TEMPLATE_Client/website/`.
...
- Ryan's instruction: mode (**CLIENT** or **PROSPECT**), company name, website address.
...
Written into `80_Clients/<client-slug>/website/` (prospects: `80_Clients/_prospects/<slug>/website/`):
...
Should this later run inside John's website intake (before the Website Builder builds), or stay a Ryan-triggered step for client projects? Which prospect folders may be shared with the prospect (today: none, they stay in the vault)?
