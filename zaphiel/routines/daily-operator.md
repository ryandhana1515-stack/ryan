...
START HERE, in order:
1. Read CLAUDE.md and the vault `zaphiel/vault/00 Home.md` (+ the section notes) in this repo. The vault is the source of truth; it outranks anything you assume.
2. Read the vault note `2b WHAT CAN ACTUALLY ACT` ("WHAT CAN ACTUALLY ACT") before promising or attempting any automation. n8n holds only Gmail and GitHub credentials — n8n workflows cannot touch Shopify, Meta, Metricool, Higgsfield or Kling. YOU are the executor; use your own MCP connectors.
...
THEN CLOSE THE LOOP:
- Append what actually changed to the vault (`zaphiel/vault/06 Change log.md`, and `05 Decisions log.md` if you decided something), commit and push to the branch claude/ai-agent-board-advisors-wfrhh1. Memory that isn't written down died with the session.
...
- **notifications:** `{ "push": true, "email": true }`
- **prompt shape:** read CLAUDE.md and zaphiel/vault/00 Home.md → read the `jarvis_tasks` table via
