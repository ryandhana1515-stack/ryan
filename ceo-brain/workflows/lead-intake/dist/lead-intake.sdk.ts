...
const loadAtlasQs = node({
  ...
  config: {
    ...
    parameters: { resource: 'row', operation: 'get', dataTableId: {"__rl":true,"mode":"id","value":"sPnRGXe4VYDJLJHr","cachedResultName":"ceo_tasks"}, matchType: 'allConditions', filters: { conditions: [{ keyName: 'task_type', condition: 'eq', keyValue: 'edg_design' }, { keyName: 'lead_id', condition: 'eq', keyValue: expr("{{ $('Resolve Lead Identity').item.json.lead.lead_id }}") }] }, returnAll: false, limit: 3 },
    position: [1670, 480]
  },
  output: [{}]
});
...
const edgGate = ifElse({
  ...
  config: {
    ...
    parameters: {
      conditions: {
        options: { caseSensitive: true, leftValue: '', typeValidation: 'loose', version: 2 },
        conditions: [{ id: 'edg', leftValue: expr("{{ $('Finalize & Validate Result').item.json.edg_requested }}"), rightValue: true, operator: { type: 'boolean', operation: 'true', singleValue: true } }],
        combinator: 'and'
      }
    },
    position: [4310, 80]
...
const callAtlas = node({
  ...
  config: {
    ...
    parameters: {
      mode: 'once',
      source: 'database',
      workflowId: { __rl: true, mode: 'id', value: "9XQWSgTBszRc0Jxj", cachedResultName: 'CEO Brain — ATLAS (EDG & CRM Systems Architect)' },
      workflowInputs: {
        mappingMode: 'defineBelow',
        value: {
          tenant_id: expr("{{ $('Finalize & Validate Result').item.json.lead.tenant_id }}"),
