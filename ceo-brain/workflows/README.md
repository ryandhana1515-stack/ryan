...
- **Website Builder** is generated by `website-builder/build.js`, the **Discovery Console** by
  `discovery-console/build.js`, the **CEO Orchestrator** by `ceo-orchestrator/build.js` and the **Approval
  Inbox** by `approval-inbox/build.js` (both inline `agents/ceo-orchestrator/orchestrator.js`) (same pattern: agent JS + prompts + schemas inlined). **John Chat Console** is
