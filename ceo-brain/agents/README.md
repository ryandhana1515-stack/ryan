...
| `website-intelligence` | **live, v1.0.0** | Internal agent (ADR-3) between John and the Website Builder. Identifies the company, searches and reads its public pages (n8n Gateway credits: Browserbase), labels every fact, hands the Website Builder a WEBSITE_CREATOR_BRIEF with placeholders; questions go to John through the Orchestrator; never talks to the customer; never delays a mock-up. Role prompt (Ryan, verbatim) read live from the vault. |
