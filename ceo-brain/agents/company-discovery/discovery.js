// Company Discovery & Onboarding Agent — pure functions, no n8n globals, ES5-compatible.
// Converts an ordinary conversation with a business owner into the Client Digital Company Map,
// progressively (never 100 questions at once) and without ever asking for "all your data".
// Inlined into the Discovery Console Code nodes by workflows/discovery-console/build.js.
...
// The conversation plan: one opening question per topic, in the order a good consultant would ask.
var DS_QUESTIONS = {
...
// ---- Rules extraction (fallback when the model is unavailable). Facts only from the owner's words. ----
var DS_SOFTWARE = [
...
// The topic the previous fallback turn asked about is deterministic (first open topics of the stored map),
// so the owner's answer is recorded verbatim under that topic — their words, never an interpretation.
...
function dsFallbackTurn(input) {
  ...
  // Facts already established are never overwritten by a later keyword match (e.g. 'agency' in turn 3).
...
// Guardrails: the reply must never ask for passwords/credentials or "all your data", never quote prices or guarantees.
...
function finalizeDiscoveryTurn(opts) {
  ...
  // Facts guard: a company name the owner never typed is dropped.
...
module.exports = { DS_VERSION, DS_TOPICS, DS_QUESTIONS, DS_MODULES, DS_AGENTS, DS_COMPLETE_AT, DS_MIN_TURNS, dsRecordAnswer, dsEmptyMap, dsCoerceMap, dsMergeMap, dsTopicStatus, dsNextTopics, dsExtractPatch, dsFallbackTurn, dsCoerceTurn, dsValidateTurn, dsParseJson, finalizeDiscoveryTurn, dsRenderMapNote };
