...
- Convene: run n8n workflow `NOb10f0yUA8i8saA` with {question, brief_short, brief_full,
  unprompted, ref, budget_usd}. The chair (only) sees the full live brief; seats are
  isolated one-dossier calls; citations are gated server-side; every meeting is stored
  in `board_meetings` and emailed to ryandhana1515@gmail.com. Cost ceiling default $0.50.
